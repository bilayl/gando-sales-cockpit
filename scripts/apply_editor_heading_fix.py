...
# SD01: remove the redundant in-page SD01–SD05 card navigation.
path = ROOT / "components/sd-room-workspace.tsx"
text = path.read_text()
old = 'tab === "content" ? <SDRoomEditor dealId={dealId} />'
new = 'tab === "content" ? <div className="sd01-workspace"><SDRoomEditor dealId={dealId} /><style jsx global>{`.sd01-workspace nav[aria-label="Parcours SD"] { display: none !important; }`}</style></div>'
if old not in text:
    raise RuntimeError("SD01 workspace anchor not found")
path.write_text(text.replace(old, new, 1))

# SD05 builder: replace marker-appending buttons with formatting on the current line.
...
# SD05 renderer: headings must never absorb the paragraph below them.
