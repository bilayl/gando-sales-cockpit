...
# --- SD05 builder: make H2/H3/H4 formatting act on the current line instead of appending broken markers.
...
area_anchor = '''function Area({ value, onChange, rows = 5, placeholder, disabled = false }: { value: string; onChange: (value: string) => void; rows?: number; placeholder?: string; disabled?: boolean }) {
...
function ContractTextEditor({ value, onChange, disabled, onAddAnnex }: { value: string; onChange: (value: string) => void; disabled: boolean; onAddAnnex: () => void }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-input bg-background focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15">
    ...
    />
    <div className="border-t border-border bg-muted/15 px-4 py-2 text-[11px] leading-5 text-muted-foreground">Place le curseur sur une ligne puis choisis son niveau. Le préfixe technique n'est jamais affiché dans le contrat final. H2 = titre, H3 = sous-titre, H4 = sous-section.</div>
  </div>;
}
'''
...
replacement = '''
      <Card className="space-y-4 p-5">
        <div><h2 className="font-semibold">Texte du contrat</h2><p className="mt-1 text-xs leading-5 text-muted-foreground">Éditeur structuré : appliquez directement un niveau à la ligne courante. Les titres et sous-titres restent distincts du texte, même sans ligne vide. « ARTICLE », « 3.1. » et « ANNEXE » restent également reconnus.</p></div>
...
# --- Renderer: heading lines must not swallow the paragraph immediately below them.
path = ROOT / "components/sd05-contract-renderer.tsx"
text = path.read_text()
paginate_pattern = re.compile(r'function paginate\(body: string\): RenderBlock\[\]\[\] \{.*?\n\}\n\nfunction ContractBlock', re.S)
new_paginate = '''function renderSegments(raw: string): RenderBlock[] {
...
function paginate(body: string): RenderBlock[][] {
  // Keep the legacy blank-block pagination so existing signature page counts remain stable.
  // Only rendering inside each block is refined to separate H2/H3/H4 from the following paragraph.
...
text, count = paginate_pattern.subn(new_paginate, text, count=1)
if count != 1:
    raise RuntimeError(f"Renderer paginate replacement failed: {count}")
path.write_text(text)

print("SD05 heading editor and renderer fixed")
