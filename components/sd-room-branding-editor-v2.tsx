    ...
    <div className={cn(embedded ? "" : "page-shell min-h-screen p-5 lg:p-7")}>
      <div className={cn("mx-auto space-y-5", embedded ? "max-w-none" : "max-w-[1280px]")}>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div><div className="text-[10px] font-black uppercase tracking-[0.16em] text-primary">{embedded ? "Branding de la propal" : "Site client"}</div><h1 className={cn("mt-1 font-black tracking-[-0.035em]", embedded ? "text-xl" : "text-2xl")}>{embedded ? "Bannière client" : "Branding de la Deal Room"}</h1><p className="mt-1 text-sm text-muted-foreground">{embedded ? "Cette bannière est affichée sur le lien public du Deal rapide." : "Cette bannière est exactement celle affichée sur le lien public de la Deal Room."}</p></div>
