...
export function KpiWorkspace({ view, canEdit }: { view: KpiView; canEdit: boolean }) {
  if (view === "overview") {
    return <div className="min-w-0 p-4 lg:p-5"><KpiExecutiveOverview /></div>
  }

  if (view === "funnel") {
    return <div className="min-w-0 p-4 lg:p-5"><KpiFunnelShadcn canEdit={canEdit} /></div>
  }

  if (view === "history") {
    return <div className="min-w-0 p-4 lg:p-5"><KpiMonthlyShadcn canEdit={canEdit} /></div>
  }
