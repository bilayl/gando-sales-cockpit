...
function isFilled(row: KpiRow) { return EDIT_FIELDS.some(field => row[field.key] != null); }
...
function averageGrowth(rows: KpiRow[], key: NumericKey) {
  ...
  for (let i = 1; i < sorted.length; i += 1) {
    ...
    const p = prev[key];
    const c = current[key];
    if (monthIndex(current) - monthIndex(prev) === 1 && typeof p === "number" && p > 0 && typeof c === "number") rates.push(c / p - 1);
  }
  return mean(rates);
}
function averageIncrement(rows: KpiRow[], key: NumericKey) {
  ...
  for (let i = 1; i < sorted.length; i += 1) {
    ...
    const p = prev[key];
    const c = current[key];
    if (monthIndex(current) - monthIndex(prev) === 1 && typeof p === "number" && typeof c === "number") values.push(c - p);
  }
  return mean(values);
}
...
function NumberField({ label, value, suffix, percentValue, onChange }: {
  label: string; value: number | null | undefined; suffix?: string; percentValue?: boolean; onChange: (value: number | null) => void;
}) {
  return (
    <label className="space-y-1.5">
      ...
      <div className="relative">
        <Input
          type="number"
          step="any"
          value={value == null ? "" : String(percentValue ? value * 100 : value)}
          onChange={event => {
...
export function BusinessKpiDashboard({ canEdit }: { canEdit: boolean }) {
  ...
  const lastClosed = actualRows.at(-1) || null;

  const historical = useMemo(() => {
    const churnRows = mean(actualRows.map(row => row.churnRate));
    return {
      ...
      arpu: averageRatio(actualRows, "revenue", "activeRenters"),
      takeRate: averageRatio(actualRows, "revenue", "tdv"),
      tdvPerDeposit: averageRatio(actualRows, "tdv", "deposits"),
      depositsPerRenter: averageRatio(actualRows, "deposits", "activeRenters"),
      depositGrowth: averageGrowth(actualRows, "deposits"),
      newClientsPerMonth: averageIncrement(actualRows, "totalClients"),
      cashoutRate: averageRatio(actualRows, "depositCashouts", "deposits"),
      cashoutAmount: averageRatio(actualRows, "cashoutAmount", "depositCashouts"),
      guaranteeShare: averageRatio(actualRows, "advancedGuarantee", "cashoutAmount"),
      churnRate: churnRows || averageRatio(actualRows, "churnedRenters", "activeRenters") || 0,
    };
  }, [actualRows]);
  ...
  useEffect(() => {
    if (selectedMonth || !rows.length) return;
    const now = new Date();
    setSelectedMonth(rowKey(now.getFullYear(), now.getMonth() + 1));
  }, [rows, selectedMonth]);
  ...
  async function save() {
    ...
    try {
      const response = await fetch("/api/kpi", { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(draft) });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error || "Enregistrement impossible");
      setRows(Array.isArray(body.rows) ? body.rows : rows);
      setEditorOpen(false); setMessage("Données enregistrées");
  ...
  const actualCards: Array<{ label: string; value: string; helper: string; icon: IconType }> = [
  ...
  const averages: Array<[string, string]> = [
    ["Revenu moyen / mois", euro(historical.revenue, 2)], ["TDV moyen / mois", euro(historical.tdv)],
    ["Cautions / mois", decimal(historical.deposits)], ["MAU moyen", decimal(historical.mau)],
    ["ARPU moyen", euro(historical.arpu, 2)], ["Take rate moyen", percent(historical.takeRate, 2)],
    ["TDV / caution", euro(historical.tdvPerDeposit, 2)], ["Cautions / loueur", decimal(historical.depositsPerRenter, 2)],
    ["Nouveaux users / mois", decimal(historical.newUsers)], ["Croissance cautions", percent(historical.depositGrowth)],
  ];

  const projectionCards: Array<{ label: string; value: string; icon: IconType }> = projected ? [
    { label: "Revenu projeté", value: euro(projected.revenue, 2), icon: Banknote },
  ...
  return (
    <div>
      <section className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-semibold text-emerald-700 dark:border-emerald-900/70 dark:bg-emerald-950/30 dark:text-emerald-300"><TrendingUp className="h-3.5 w-3.5" /> Pilotage entreprise</div>
          <h1 className="text-3xl font-semibold tracking-[-0.04em]">Business KPI</h1>
          <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Saisissez le réel puis projetez Gando avec ses moyennes historiques.</p>
        </div>
        <div className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white p-1 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          ...
          <Button size="sm" variant={mode === "simulation" ? "secondary" : "ghost"} onClick={() => setMode("simulation")}><Calculator className="mr-1.5 h-4 w-4" /> Simulation</Button>
        </div>
      </section>
      ...
      {mode === "real" ? <>
        <section className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {actualCards.map(card => { const Icon = card.icon; return <article key={card.label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-white/[0.04]"><div className="flex items-center justify-between"><span className="text-xs font-medium text-slate-500">{card.label}</span><span className="grid h-8 w-8 place-items-center rounded-lg bg-slate-100 text-slate-600 dark:bg-white/[0.06]"><Icon className="h-4 w-4" /></span></div><div className="mt-5 text-[25px] font-semibold tracking-[-0.04em]">{card.value}</div><div className="mt-1 text-[11px] text-slate-400">{card.helper}</div></article>; })}
        </section>
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 dark:border-white/10"><div><h2 className="text-base font-semibold">Suivi mensuel</h2><p className="mt-1 text-xs text-slate-400">Cliquez sur un mois pour saisir ou corriger ses données.</p></div>{canEdit ? <Button size="sm" onClick={() => setEditorOpen(value => !value)}><Pencil className="mr-1.5 h-4 w-4" /> Saisir un mois</Button> : null}</div>
          {editorOpen && canEdit && draft ? <div className="border-b border-slate-100 bg-slate-50/70 p-5 dark:border-white/10 dark:bg-white/[0.02]">
            <div className="flex flex-wrap items-end justify-between gap-3"><label className="space-y-1.5"><span className="text-[11px] font-semibold text-slate-500">Mois à saisir</span><Input type="month" value={selectedMonth} onChange={event => setSelectedMonth(event.target.value)} className="h-10 w-44 bg-white" /></label><span className="text-xs text-slate-400">Vide = donnée inconnue, pas zéro.</span></div>
            <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{EDIT_FIELDS.map(field => <NumberField key={field.key} label={field.label} value={draft[field.key] as number | null} suffix={field.suffix} percentValue={field.percent} onChange={value => setDraft(current => current ? ({ ...current, [field.key]: value } as KpiRow) : current)} />)}</div>
            <div className="mt-5 flex items-center justify-end gap-3">{message ? <span className="text-xs text-slate-500">{message}</span> : null}<Button variant="outline" onClick={() => setEditorOpen(false)}>Annuler</Button><Button onClick={save} disabled={saving}>{saving ? <RefreshCw className="mr-1.5 h-4 w-4 animate-spin" /> : <Save className="mr-1.5 h-4 w-4" />}Enregistrer</Button></div>
          </div> : null}
          <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left"><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.1em] text-slate-400 dark:bg-white/[0.025]"><tr><th className="px-5 py-3">Mois</th><th className="px-4 py-3">Revenu</th><th className="px-4 py-3">TDV</th><th className="px-4 py-3">Take rate</th><th className="px-4 py-3">Cautions</th><th className="px-4 py-3">MAU</th><th className="px-4 py-3">ARPU</th><th className="px-4 py-3">Nouveaux users</th><th className="px-4 py-3">Croissance</th><th className="px-4 py-3" /></tr></thead><tbody className="divide-y divide-slate-100 text-sm dark:divide-white/10">{sortedRows.map(row => {
            const future = !isFilled(row); const tr = row.revenue != null && row.tdv ? row.revenue / row.tdv : null; const rowArpu = row.revenue != null && row.activeRenters ? row.revenue / row.activeRenters : null;
            return <tr key={rowKey(row.year, row.monthNumber)} className={cn(future && "text-slate-300 dark:text-slate-600", !future && "hover:bg-slate-50/70 dark:hover:bg-white/[0.02]")}><td className="px-5 py-3.5 font-medium">{row.month} <span className="ml-1 text-xs font-normal text-slate-400">{row.year}</span></td><td className="px-4 py-3.5 font-medium">{euro(row.revenue, 2)}</td><td className="px-4 py-3.5">{euro(row.tdv)}</td><td className="px-4 py-3.5">{percent(tr)}</td><td className="px-4 py-3.5">{integer(row.deposits)}</td><td className="px-4 py-3.5">{integer(row.activeRenters)}</td><td className="px-4 py-3.5">{euro(rowArpu, 2)}</td><td className="px-4 py-3.5">{integer(row.newUsers)}</td><td className="px-4 py-3.5">{percent(row.growth)}</td><td className="px-4 py-3.5 text-right">{canEdit ? <Button size="sm" variant="ghost" onClick={() => editRow(row)}><Pencil className="h-3.5 w-3.5" /></Button> : null}</td></tr>;
          })}</tbody></table></div>
        </section>
      </> : assumptions ? <>
        <section className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          <div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-lg font-semibold">Moyennes historiques</h2><p className="mt-1 max-w-2xl text-xs leading-5 text-slate-400">Le moteur repart de novembre 2025 et rejoue l’activité avec les moyennes observées. Tu peux modifier chaque hypothèse.</p></div><div className="flex gap-1 rounded-lg bg-slate-100 p-1 dark:bg-white/[0.05]">{[6,12,24].map(value => <Button key={value} size="sm" variant={horizon === value ? "secondary" : "ghost"} onClick={() => setHorizon(value)}>+{value} mois</Button>)}</div></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{averages.map(([label, value]) => <div key={label} className="rounded-xl bg-slate-50 px-3 py-3 dark:bg-white/[0.035]"><div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">{label}</div><div className="mt-1.5 text-lg font-semibold tracking-[-0.02em]">{value}</div></div>)}</div>
          <div className="mt-6 border-t border-slate-100 pt-5 dark:border-white/10"><div className="mb-3 text-xs font-semibold text-slate-500">Hypothèses de simulation</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <AssumptionField label="TDV / caution" value={assumptions.tdvPerDeposit} suffix="€" onChange={value => setAssumptions({ ...assumptions, tdvPerDeposit: value })} />
        ...
        <section className="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-white/[0.04]">
          <div className="border-b border-slate-100 px-5 py-4 dark:border-white/10"><h2 className="text-base font-semibold">Réel vs simulation depuis le début</h2><p className="mt-1 text-xs text-slate-400">Simulation depuis le premier mois connu, puis projection +{horizon} mois après le dernier mois réel.</p></div>
          <div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-left"><thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-[0.08em] text-slate-400 dark:bg-white/[0.025]"><tr><th className="px-5 py-3">Mois</th><th className="px-4 py-3">Revenu réel</th><th className="px-4 py-3">Revenu simulé</th><th className="px-4 py-3">TDV réel</th><th className="px-4 py-3">TDV simulé</th><th className="px-4 py-3">Cautions réel</th><th className="px-4 py-3">Cautions simulé</th><th className="px-4 py-3">MAU réel</th><th className="px-4 py-3">MAU simulé</th></tr></thead><tbody className="divide-y divide-slate-100 text-sm dark:divide-white/10">{simulation.map(row => <tr key={rowKey(row.year, row.monthNumber)} className={lastClosed && monthIndex(row) > monthIndex(lastClosed) ? "bg-violet-50/35 dark:bg-violet-950/10" : ""}><td className="px-5 py-3.5 font-medium">{row.month} <span className="ml-1 text-xs font-normal text-slate-400">{row.year}</span></td><td className="px-4 py-3.5">{euro(row.actual?.revenue, 2)}</td><td className="px-4 py-3.5 font-semibold text-violet-700 dark:text-violet-300">{euro(row.revenue, 2)}</td><td className="px-4 py-3.5">{euro(row.actual?.tdv)}</td><td className="px-4 py-3.5 font-medium">{euro(row.tdv)}</td><td className="px-4 py-3.5">{integer(row.actual?.deposits)}</td><td className="px-4 py-3.5 font-medium">{integer(row.deposits)}</td><td className="px-4 py-3.5">{integer(row.actual?.activeRenters)}</td><td className="px-4 py-3.5 font-medium">{integer(row.activeRenters)}</td></tr>)}</tbody></table></div>
        </section>
      </> : null}
    </div>
  );
