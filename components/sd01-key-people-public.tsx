...
export function SD01KeyPeoplePublic({ stakeholders }: { stakeholders: Stakeholder[] }) {
  ...
  return <section className="rounded-[22px] border border-[#e2e7e4] bg-white px-5 py-5 shadow-[0_10px_35px_rgba(20,35,28,0.035)] sm:px-7 sm:py-6">
    <div className="flex items-end justify-between gap-4 border-b border-[#edf0ee] pb-4">
      <div>
        <div className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#735DF3]">Interlocuteurs & décideurs</div>
        <h2 className="mt-1 text-xl font-bold tracking-[-0.025em] text-[#18221e]">Personnes clés</h2>
      </div>
      <div className="hidden items-center gap-1.5 text-[11px] font-semibold text-[#7b8781] sm:flex"><UserRound className="h-3.5 w-3.5" /> {people.length} personne{people.length > 1 ? "s" : ""}</div>
    </div>
      ...
      {people.map((person, index) => <article key={`${person.name}-${person.role}-${index}`} className="rounded-2xl border border-[#e8ecea] bg-[#fafbfa] p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h3 className="truncate text-sm font-bold text-[#18221e]">{person.name || "Interlocuteur à confirmer"}</h3>
