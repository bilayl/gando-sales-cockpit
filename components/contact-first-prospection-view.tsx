...
export function ContactFirstProspectionView() {
  ...
  async function load(silent = false, forceRecommendationRefresh = false) {
    ...
    try {
      if (isRecommendationSegment) {
        const params = new URLSearchParams({ bucket: "ACTIONABLE", limit: "2000" });
  ...
  const pageTitle = isRecommendationSegment ? "Suggestions d’appels" : currentList?.name || "Prospection par contact";
    ...
    <div className="page-shell flex h-screen flex-col overflow-hidden">
      <div className="flex shrink-0 items-end gap-1.5 border-b border-border bg-card px-5 pt-3 minari-scrollbar">
        ...
        <div className="flex min-w-0 flex-1 items-end gap-1 overflow-x-auto">
          <button
            onClick={() => setSegmentId(CALL_RECOMMENDATIONS_SEGMENT)}
        ...
        <Card className="flex h-full min-h-0 flex-col overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3.5">
            <div>
              ...
              {isRecommendationSegment ? (
                <p className="mt-0.5 text-xs text-muted-foreground"><strong className="text-primary">{actionableCount} contacts réellement notés à appeler</strong> · {opportunities} RDV/opportunités sortis de la file · {snoozed} relances futures · {excluded} exclus</p>
              ) : (
            ...
            <div className="flex items-center gap-2">
              {isRecommendationSegment ? (
                <div className="hidden rounded-lg border border-primary/20 bg-primary/[0.05] px-3 py-2 text-xs text-muted-foreground xl:block">
                  <strong className="text-primary">Score persistant Supabase</strong> · statuts, relances, appels et tâches sont enregistrés dans le calcul{recommendationEvaluatedAt ? ` · ${formatDate(recommendationEvaluatedAt)}` : ""}.
                </div>
              ) : null}
          ...
          <div className="flex flex-wrap items-center gap-2 border-y border-border bg-muted/30 px-4 py-2.5">
            ...
            {isRecommendationSegment ? (
              <Badge variant="outline" className="h-9 gap-1.5 border-primary/20 bg-card px-3 text-primary"><Database size={13} /> Classement par score /100</Badge>
            ) : (
            ...
            <span className="ml-auto text-[11px] text-muted-foreground">{isRecommendationSegment ? "Cette liste vient de la base de données, pas d’un tri visuel du tableau." : "Les priorités du segment restent calculées depuis les propriétés HubSpot."}</span>
          </div>
                        ...
                        <TableCell>
                          <div className="min-w-[210px]">
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              {isRecommendationSegment ? <Badge variant="outline" className="border-primary/20 text-primary">{score}/100</Badge> : null}
                            </div>
                            <div className="mt-1 text-[11px] font-semibold leading-4 text-primary">{decision.suggestion}</div>
                            {isRecommendationSegment ? <div className="mt-1 max-w-[260px] text-[10px] leading-4 text-muted-foreground">{decision.reason}</div> : null}
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="flex items-center gap-2">
