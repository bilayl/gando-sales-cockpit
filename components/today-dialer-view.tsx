...
export function TodayDialerView() {
  ...
  async function openCallWorkspace() {
    if (!selected) return;
    ...
    try {
      ...
      const nextAllo = await statusResponse.json();
      setAllo(nextAllo);
      setCallContact(selected);
      setMessage("");
    } catch (error) {
          ...
          <div className="grid min-h-[520px] gap-4 xl:grid-cols-[1fr_360px]">
                ...
                <div className="grid md:grid-cols-[1fr_310px]">
                  <div className="p-7">
                    ...
                    <div className="mt-7 grid gap-3 sm:grid-cols-2">
                      <div className="rounded-xl border border-border px-4 py-3"><div className="text-[11px] text-muted-foreground">Téléphone</div><div className="mt-1 text-[14px] font-medium">{numberFor(selected) || "Aucun numéro"}</div></div>
                    ...
                    <div className="mt-7 flex flex-wrap gap-2">
                      <Button className="h-11 rounded-xl px-5" onClick={() => void openCallWorkspace()} disabled={queueing || !alloHealthy}>
                    ...
                    <p className="mt-3 max-w-2xl text-[11px] leading-5 text-muted-foreground">Le clic ouvre désormais le poste d’appel directement dans le Cockpit. Le prospect est synchronisé avec Allo avant l’ouverture de l’interface.</p>
                  </div>
            ...
            <aside className="rounded-2xl border border-border bg-card p-5">
              ...
              <div className="mt-5 space-y-4 text-[12px]">
                ...
                <div><div className="text-muted-foreground">Power Dialer</div><div className="mt-1 font-medium">{allo?.powerDialerReady ? `Prêt · ${allo?.queue?.count || 0} numéro(s)` : "Scope DIALING_QUEUE_READ_WRITE manquant"}</div></div>
                <div><div className="text-muted-foreground">Audio dans le Cockpit</div><div className="mt-1 font-medium">{allo?.directCallReady ? "Capacité directe détectée" : "Interface intégrée, média Allo externe pour l’instant"}</div></div>
              </div>
              {!alloHealthy ? <div className="mt-5 rounded-xl border border-destructive/20 bg-destructive/10 p-3 text-[11px] leading-5 text-destructive">{allo?.error?.message || allo?.queueError?.message || "La clé Allo est présente mais le Power Dialer n’est pas utilisable avec ses scopes actuels."}</div> : null}
            </aside>
          </div>
        </div>
      </div>

      {callContact ? <InWebCallWorkspace contact={callContact} allo={allo} onClose={() => setCallContact(null)} /> : null}
    </>
  );
}
