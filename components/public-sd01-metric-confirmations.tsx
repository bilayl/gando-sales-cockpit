...
export function PublicSD01MetricConfirmations({ token, metrics, email, firstName, lastName, language, onConfirmed }: Props) {
  ...
  async function confirm(index: number, metric: SD01Metric) {
            ...
            <div key={`${metric.lever}-${index}`} className={`rounded-2xl border p-4 ${confirmed ? "border-[#d8e8dc] bg-[#f4faf5]" : "border-[#dedaf7] bg-[#f8f7ff]"}`}>
              ...
              {confirmed ? (
                ...
                <div className="mt-4">
                  ...
                  <button type="button" onClick={() => void confirm(index, metric)} disabled={working === index} className="mt-3 inline-flex h-9 items-center gap-2 rounded-lg bg-[#202a2f] px-3.5 text-[12px] font-semibold text-white disabled:opacity-50">
