...
import { CheckCircle2, Copy, ExternalLink, Loader2, Save, Send } from "lucide-react";
...
export function SDQuickProposalBuilder({ dealId, onChanged }: { dealId: string; onChanged?: () => void }) {
  ...
  async function persist(publish: boolean) {
    ...
    try {
      ...
      toast.success(publish ? "Propal envoyée dans la Deal Room" : "Propal enregistrée");
      onChanged?.();
  ...
  return <div className="page-shell min-h-screen p-5 lg:p-7"><div className="mx-auto max-w-[1080px] space-y-5">
    <Card className="p-5 lg:p-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div><div className="text-[11px] font-black uppercase tracking-[0.14em] text-primary">Deal rapide · Étape 1</div><h1 className="mt-1 text-2xl font-black tracking-[-0.03em]">Propal en ligne</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Une proposition courte, modifiable en quelques secondes. Le client peut l’ouvrir et confirmer directement son accord.</p></div>
        <div className="flex flex-wrap gap-2">{agreed ? <Badge className="border-emerald-500/25 bg-emerald-500/10 text-emerald-600" variant="outline"><CheckCircle2 className="mr-1 h-3.5 w-3.5" /> Accord obtenu</Badge> : published ? <Badge variant="outline">Envoyée · en attente</Badge> : <Badge variant="outline">Brouillon</Badge>}</div>
      </div>
      <div className="mt-5 flex flex-wrap gap-2 border-t border-border pt-4"><Button variant="outline" onClick={() => void persist(false)} disabled={working}><Save className="mr-2 h-4 w-4" />Enregistrer</Button><Button onClick={() => void persist(true)} disabled={working || !ready || agreed}>{working ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}{published ? "Renvoyer la propal" : "Envoyer au client"}</Button>{clientUrl && published ? <Button variant="outline" onClick={async () => { await navigator.clipboard.writeText(clientUrl); toast.success("Lien client copié"); }}><Copy className="mr-2 h-4 w-4" />Copier le lien</Button> : null}{clientUrl && published ? <Button variant="ghost" asChild><a href={clientUrl} target="_blank" rel="noreferrer">Voir côté client <ExternalLink className="ml-2 h-4 w-4" /></a></Button> : null}</div>
    </Card>

    <div className="grid gap-5 lg:grid-cols-[1.1fr_.9fr]">
      <Card className="space-y-5 p-5 lg:p-6">
        <Field label="Titre de la proposition"><Input value={value.deckTitle} onChange={event => set("deckTitle", event.target.value)} placeholder="ex : Proposition Gando × LR Location" /></Field>
        <Field label="Message principal" hint="Ce que le client doit comprendre en 20 secondes"><Area value={value.executiveMessage} onChange={next => set("executiveMessage", next)} rows={6} placeholder="Nous vous proposons…" /></Field>
        <Field label="Ce que comprend l’offre" hint="Un élément par ligne"><Area value={value.solution.join("\n")} onChange={next => set("solution", lines(next))} rows={7} placeholder={'Caution sans blocage de fonds\nGarantie d’encaissement\n60 jours de sécurisation'} /></Field>
        <Field label="Prochaine étape"><Input value={value.callToAction} onChange={event => set("callToAction", event.target.value)} placeholder="Êtes-vous en accord avec cette proposition ?" /></Field>
      </Card>

      <Card className="space-y-5 p-5 lg:p-6">
        <Field label="Prix / offre" hint="Une ligne : intitulé | prix | précision"><Area value={pricingText(value.pricing)} onChange={next => set("pricing", parsePricing(next))} rows={7} placeholder={'Tarif Gando | 2,7 % HT | par caution\nMarge partenaire | +0,7 % HT | conservée par le loueur'} /></Field>
        <Field label="Conditions commerciales" hint="Une condition par ligne"><Area value={value.commercialTerms.join("\n")} onChange={next => set("commercialTerms", lines(next))} rows={7} placeholder={'60 jours de sécurisation\nFrais d’encaissement : 3,5 % + 2 € HT\nGarantie selon conditions contractuelles'} /></Field>
        <Field label="Points de valeur / ROI" hint="Une ligne par bénéfice"><Area value={value.proofPoints.join("\n")} onChange={next => set("proofPoints", lines(next))} rows={6} placeholder={'Pas de fonds bloqués pour le locataire\nGain de temps pour les agents\nRevenu additionnel sur chaque caution'} /></Field>
      </Card>
    </div>
  </div></div>;
}