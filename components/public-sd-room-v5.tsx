...
import { ArrowUpRight, Check, CheckCircle2, ChevronRight, ExternalLink, FileSignature, Loader2, LockKeyhole, MessageSquare, ShieldCheck } from "lucide-react";
...
function Section({ title, children, kicker }: { title: string; children: React.ReactNode; kicker?: string }) {
  return <section className="rounded-[18px] border border-[#e0e4e6] bg-white px-5 py-6 shadow-[0_1px_2px_rgba(20,30,35,0.025)] sm:px-8 sm:py-8">{kicker ? <Eyebrow>{kicker}</Eyebrow> : null}<h2 className="mt-1 text-[21px] font-semibold tracking-[-0.025em] text-[#172126] sm:text-[23px]">{title}</h2><div className="mt-5 text-[15px] leading-7 text-[#465157]">{children}</div></section>;
}
...
function PairGrid({ leftTitle, left, rightTitle, right }: { leftTitle: string; left: React.ReactNode; rightTitle: string; right: React.ReactNode }) {
  return <div className="grid gap-6 lg:grid-cols-2"><div><h3 className="mb-3 text-[12px] font-semibold uppercase tracking-[0.08em] text-[#6f797e]">{leftTitle}</h3>{left}</div><div><h3 className="mb-3 text-[12px] font-semibold uppercase tracking-[0.08em] text-[#6f797e]">{rightTitle}</h3>{right}</div></div>;
}
...
function SD04Document({ content }: { content: SD04Content }) {
  const executive = content.executiveMessage || content.offerSummary;
  return <div className="space-y-5 sm:space-y-6">
    <Section title={content.deckTitle || "Offre commerciale"} kicker="SD04 · Offre"><p className="text-[18px] font-medium leading-8 text-[#202a2f]">{executive || content.deckSubtitle || "Offre à finaliser."}</p>{content.deckSubtitle && executive ? <p className="mt-3 text-[#687277]">{content.deckSubtitle}</p> : null}</Section>
    <Section title="Pourquoi maintenant ?"><PairGrid leftTitle="Enjeux client" left={<BulletList items={content.problem} />} rightTitle="Réponse Gando" right={<BulletList items={content.solution} />} /></Section>
    <Section title="Pourquoi Gando ?"><PairGrid leftTitle="Différenciation" left={<BulletList items={content.differentiators} />} rightTitle="Preuves" right={<BulletList items={content.proofPoints} />} /></Section>
    {content.businessCase?.length ? <Section title="Impact / ROI"><div className="divide-y divide-[#eceeef]">{content.businessCase.map((row, index) => <div key={index} className="grid gap-3 py-4 first:pt-0 last:pb-0 sm:grid-cols-[1fr_1fr] text-sm"><div className="font-semibold text-[#202a2f]">{row.metric}</div><div><div>Actuel : {row.baseline || "—"}</div><div>Cible : {row.target || "—"}</div>{row.value ? <div className="mt-1 font-semibold text-[#5d52b6]">{row.value}</div> : null}</div></div>)}</div></Section> : null}
    <Section title="Offre et tarification">{content.pricing?.length ? <div className="divide-y divide-[#eceeef]">{content.pricing.map((row, index) => <div key={index} className="grid gap-2 py-4 first:pt-0 last:pb-0 sm:grid-cols-[1fr_1fr_140px]"><div className="font-semibold text-[#202a2f]">{row.item}</div><div>{row.model}</div><div className="font-semibold text-[#5d52b6]">{row.price}</div>{row.notes ? <div className="text-sm text-[#687277] sm:col-span-3">{row.notes}</div> : null}</div>)}</div> : <p className="italic text-[#81898e]">Tarification à finaliser.</p>}{content.validityDate ? <p className="mt-5 border-t border-[#eceeef] pt-4 text-sm text-[#687277]">Offre valable jusqu’au {formatDate(content.validityDate)}.</p> : null}</Section>
    {content.callToAction ? <Section title="Décision attendue"><p className="text-[17px] font-semibold leading-7 text-[#202a2f]">{content.callToAction}</p></Section> : null}
  </div>;
}
...
function SD05Document({ content }: { content: SD05Content }) {
  ...
  return <div className="space-y-5 sm:space-y-6">
    <Section title={content.contractTitle || "Contrat"} kicker="SD05 · Contrat & signature">
      <div className="flex flex-col gap-5">
        <div className="flex flex-wrap items-center gap-3"><span className={`inline-flex rounded-full px-3 py-1.5 text-[12px] font-semibold ${signed ? "bg-[#edf7ef] text-[#376b43]" : ready ? "bg-[#f3f0ff] text-[#5c50ae]" : "bg-[#f1f3f4] text-[#60696e]"}`}>{signed ? "Contrat signé" : ready ? "Prêt à signer" : "Brouillon"}</span>{content.signatureDeadline ? <span className="text-sm text-[#687277]">Signature attendue avant le {formatDate(content.signatureDeadline)}</span> : null}</div>
        {content.contractSummary ? <p className="text-[16px] leading-7 text-[#465157]">{content.contractSummary}</p> : null}
        {content.contractUrl ? <a href={content.contractUrl} target="_blank" rel="noreferrer" className="inline-flex h-11 w-fit items-center gap-2 rounded-xl bg-[#202a2f] px-5 text-[13px] font-semibold text-white transition hover:bg-[#303b40]"><FileSignature className="h-4 w-4" />{signed ? "Voir le contrat signé" : "Ouvrir et signer le contrat"}<ExternalLink className="h-4 w-4" /></a> : <div className="rounded-xl border border-dashed border-[#d9dde0] bg-[#fafbfb] px-4 py-4 text-sm text-[#737d82]">Le lien du contrat sera ajouté ici.</div>}
      </div>
    </Section>
    <Section title="Signataires">{content.signatories?.length ? <div className="divide-y divide-[#eceeef]">{content.signatories.map((person, index) => <div key={index} className="flex flex-col gap-2 py-4 first:pt-0 last:pb-0 sm:flex-row sm:items-center sm:justify-between"><div><div className="font-semibold text-[#202a2f]">{person.name || person.email}</div><div className="mt-1 text-sm text-[#687277]">{[person.role, person.email].filter(Boolean).join(" · ")}</div></div><span className={`w-fit rounded-full px-2.5 py-1 text-[11px] font-semibold ${person.signatureStatus === "signed" ? "bg-[#edf7ef] text-[#376b43]" : "bg-[#f1f3f4] text-[#60696e]"}`}>{person.signatureStatus === "signed" ? "Signé" : person.signatureStatus === "sent" ? "Envoyé" : "À signer"}</span></div>)}</div> : <p className="italic text-[#81898e]">Signataires à confirmer.</p>}</Section>
  </div>;
}
...
function AccessGate({ firstName, lastName, email, setFirstName, setLastName, setEmail, loading, error, onSubmit }: { firstName: string; lastName: string; email: string; setFirstName: (value: string) => void; setLastName: (value: string) => void; setEmail: (value: string) => void; loading: boolean; error: string; onSubmit: (event: React.FormEvent) => void }) {
  return <main className="min-h-screen bg-[#f5f6f7] px-5 py-8 text-[#172126] sm:px-8 sm:py-12"><div className="mx-auto max-w-[760px]"><header className="flex items-center justify-between"><div className="flex items-center gap-2.5"><GandoMark className="h-8 w-8" /><span className="text-sm font-semibold">Gando</span></div><span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#515b60]">Deal Room privée</span></header><form onSubmit={onSubmit} className="mt-12 rounded-[22px] border border-[#d8dde0] bg-white p-6 shadow-[0_18px_55px_rgba(30,40,45,0.06)] sm:p-8"><Eyebrow>Accéder à la Room</Eyebrow><h1 className="mt-2 text-[34px] font-semibold tracking-[-0.04em] text-[#11181c]">Identifiez-vous</h1><p className="mt-3 text-[15px] leading-7 text-[#3f494e]">Ces informations permettent d’attribuer les commentaires et validations.</p><div className="mt-7 grid gap-4 sm:grid-cols-2"><label><span className="text-[12px] font-semibold text-[#202a2f]">Prénom</span><input value={firstName} onChange={event => setFirstName(event.target.value)} required autoComplete="given-name" className="mt-2 h-11 w-full rounded-xl border border-[#cfd5d9] bg-white px-3.5 text-[15px] text-[#11181c] outline-none focus:border-[#7b6fd0] focus:ring-2 focus:ring-[#7568cf]/10" /></label><label><span className="text-[12px] font-semibold text-[#202a2f]">Nom</span><input value={lastName} onChange={event => setLastName(event.target.value)} required autoComplete="family-name" className="mt-2 h-11 w-full rounded-xl border border-[#cfd5d9] bg-white px-3.5 text-[15px] text-[#11181c] outline-none focus:border-[#7b6fd0] focus:ring-2 focus:ring-[#7568cf]/10" /></label></div><label className="mt-4 block"><span className="text-[12px] font-semibold text-[#202a2f]">Email professionnel</span><input type="email" value={email} onChange={event => setEmail(event.target.value)} required autoComplete="email" className="mt-2 h-11 w-full rounded-xl border border-[#cfd5d9] bg-white px-3.5 text-[15px] text-[#11181c] outline-none focus:border-[#7b6fd0] focus:ring-2 focus:ring-[#7568cf]/10" /></label>{error ? <p className="mt-4 rounded-xl bg-[#fff3f1] px-3.5 py-3 text-[13px] text-[#9a4137]">{error}</p> : null}<button type="submit" disabled={loading} className="mt-6 flex h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#202a2f] px-4 text-[14px] font-semibold text-white transition hover:bg-[#303b40] disabled:opacity-55">{loading ? <Loader2 className="h-4 w-4 animate-spin" /> : null} Entrer dans la Room <ChevronRight className="h-4 w-4" /></button><div className="mt-4 flex items-center justify-center gap-2 text-[12px] text-[#5f696e]"><ShieldCheck className="h-4 w-4 text-[#6558c8]" /> Accès identifié et confidentiel</div></form></div></main>;
}

export function PublicSDRoomV5({ token }: { token: string }) {
  ...
  const sendComment = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setComment(""); setCommentState("sent"); window.setTimeout(() => setCommentState("idle"), 2200);
  ...
  return <main className="min-h-screen bg-[#f5f6f7] text-[#1c2529]">
    ...
    <SDRoomBrandBanner companyName={data.room.companyName} logoUrl={data.room.companyLogoUrl} bannerUrl={data.room.bannerImageUrl} theme={data.room.theme} title={data.room.displayTitle || `${data.room.companyName} × Gando`} subtitle={data.room.displaySubtitle || "Espace de collaboration"} className="border-b border-[#e1e4e6]" />

    <section className="border-b border-[#e4e7e9] bg-white"><div className="mx-auto flex max-w-[1180px] flex-wrap items-center justify-between gap-3 px-5 py-4 text-[12px] text-[#707a7f] sm:px-7"><div className="flex flex-wrap items-center gap-x-5 gap-y-2"><span>Mis à jour le {formatDate(data.room.updatedAt)}</span><span className="h-1 w-1 rounded-full bg-[#bcc2c5]" /><span>{requiredValidated}/3 étapes obligatoires validées</span></div><span>Synthèse → Plan d’action → Contrat</span></div></section>

    <div className="mx-auto grid max-w-[1180px] gap-8 px-5 py-9 sm:px-7 lg:grid-cols-[245px_minmax(0,790px)] lg:justify-between lg:py-12"><aside className="lg:sticky lg:top-24 lg:self-start"><div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#687277]">Parcours</div><nav className="mt-4 space-y-1.5">{stages.map(({ code, document }) => { const active = code === currentDocument.code; const enabled = Boolean(document); const optional = OPTIONAL_CODES.includes(code); return <button key={code} type="button" disabled={!enabled} onClick={() => enabled && setActiveStage(code)} className={`group flex w-full items-start gap-3 rounded-xl px-3 py-3 text-left transition ${active ? "bg-white shadow-sm ring-1 ring-[#e0e4e6]" : enabled ? "hover:bg-white" : "cursor-default opacity-45"}`}><span className={`mt-0.5 font-mono text-[11px] font-semibold ${active ? "text-[#584ead]" : "text-[#7f888d]"}`}>{code.slice(2)}</span><span className="min-w-0 flex-1"><span className={`block text-[13px] font-semibold leading-5 ${active ? "text-[#202a2f]" : "text-[#505b60]"}`}>{SD_STAGE_META[code].title}</span><span className="mt-0.5 block text-[11px] text-[#747e83]">{optional ? "Facultatif · " : "Obligatoire · "}{document?.status === "validated" ? "Validé" : document?.status === "published" ? "À valider" : "À venir"}</span></span>{document?.status === "validated" ? <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-[#4f835e]" /> : active ? <ChevronRight className="mt-0.5 h-4 w-4 shrink-0 text-[#6558c8]" /> : null}</button>; })}</nav></aside>

      <article className="min-w-0"><div className="mb-7 flex items-center justify-between gap-4 border-b border-[#dfe3e5] pb-5"><div><Eyebrow>{currentDocument.code}{OPTIONAL_CODES.includes(currentDocument.code) ? " · Facultatif" : " · Obligatoire"}</Eyebrow><h1 className="mt-1 text-[30px] font-semibold tracking-[-0.035em] text-[#182227]">{SD_STAGE_META[currentDocument.code].title}</h1></div><span className={`shrink-0 rounded-full px-3 py-1.5 text-[11px] font-semibold ${currentDocument.status === "validated" ? "bg-[#edf7ef] text-[#366844]" : "bg-white text-[#565f64] ring-1 ring-[#dfe3e5]"}`}>{currentDocument.status === "validated" ? "Validé" : "À valider"}</span></div><DocumentBody document={currentDocument} />

        <section className="mt-7 rounded-[18px] border border-[#dfe3e5] bg-white p-5 sm:p-7">{currentDocument.status === "validated" ? <div className="flex gap-4"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#e8f2ea] text-[#3d7049]"><Check className="h-5 w-5" /></div><div><div className="font-semibold text-[#24302c]">Cette étape est validée</div><p className="mt-1 text-[14px] leading-6 text-[#606b70]">{validatedBy ? `Validée par ${validatedBy}` : "Validation enregistrée"}{currentDocument.validated_at ? ` · ${formatDate(currentDocument.validated_at)}` : ""}.</p></div></div> : <div className="grid gap-5 sm:grid-cols-[1fr_auto] sm:items-center"><div><Eyebrow>Validation</Eyebrow><h3 className="mt-1 text-[21px] font-semibold tracking-[-0.02em] text-[#202a2f]">Confirmez-vous cette étape ?</h3><p className="mt-2 text-[14px] leading-6 text-[#606b70]">La validation enregistre votre accord sur cette version.</p></div><button type="button" onClick={() => void validateStage()} disabled={validating || currentDocument.status !== "published"} className="flex h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-[#202a2f] px-5 text-[13px] font-semibold text-white transition hover:bg-[#303b40] disabled:opacity-45">{validating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />} Valider {currentDocument.code}</button></div>}</section>

        <section className="mt-5 rounded-[18px] border border-[#e1e4e6] bg-[#eef0f2] p-5 sm:p-7"><div className="flex items-center gap-2"><MessageSquare className="h-4 w-4 text-[#6558c8]" /><h3 className="text-[15px] font-semibold text-[#263136]">Ajouter une remarque</h3></div><p className="mt-2 text-[13px] leading-6 text-[#606b70]">Question ou correction : votre message sera rattaché à {currentDocument.code}.</p><form onSubmit={sendComment} className="mt-4"><textarea value={comment} onChange={event => setComment(event.target.value)} rows={4} placeholder="Écrivez votre remarque…" className="w-full resize-y rounded-xl border border-[#cfd5d9] bg-white px-3.5 py-3 text-[15px] leading-6 text-[#202a2f] outline-none placeholder:text-[#7c858a] focus:border-[#776bd0] focus:ring-2 focus:ring-[#776bd0]/10" /><div className="mt-3 flex items-center justify-between"><span className="text-[11px] text-[#687277]">Attribué à {firstName} {lastName}</span><button type="submit" disabled={!comment.trim() || commentState === "sending"} className="flex h-9 items-center gap-2 rounded-lg border border-[#cfd4d7] bg-white px-3.5 text-[12px] font-semibold text-[#364046] transition hover:bg-[#fafbfb] disabled:opacity-50">{commentState === "sending" ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : commentState === "sent" ? <Check className="h-3.5 w-3.5" /> : <ArrowUpRight className="h-3.5 w-3.5" />}{commentState === "sent" ? "Envoyé" : "Envoyer"}</button></div></form></section>
      </article></div>
    <footer className="border-t border-[#dfe3e5] bg-white px-5 py-7 text-center text-[11px] text-[#6f797e]">Document confidentiel · {data.room.companyName} × Gando</footer>
  </main>;
}
