...
export function GandoSidebarMark() {
  return (
    <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-xl bg-[#CDDFFF] shadow-sm" aria-hidden="true">
...
export function CockpitSidebarHeader({
  section,
  canAccessKpi: _canAccessKpi = true,
}: {
  section: string;
  ...
  return (
    <SidebarMenu>
      <SidebarMenuItem>
        <SidebarMenuButton asChild size="lg" className="h-16">
          <Link href="/" title="Retour à l’accueil du Cockpit">
            <GandoSidebarMark />
            <div className="grid min-w-0 flex-1 text-left leading-tight">
              <span className="truncate text-[18px] font-semibold tracking-[-0.035em] text-[#202435] dark:text-white">Gando</span>
              <span className="truncate text-[10px] font-medium uppercase tracking-[0.16em] text-sidebar-foreground/55">
                COCKPIT · {section}
              </span>
            </div>
          </Link>
