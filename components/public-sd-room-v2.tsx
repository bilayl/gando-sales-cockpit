...
function ClientHero({ room }: { room: PublicRoomData["room"] }) {
  ...
  return (
    <section
      className={cn(
        "relative isolate w-full overflow-hidden rounded-b-[30px] bg-gradient-to-br px-5 pb-12 pt-12 shadow-[0_24px_65px_rgba(84,62,177,0.18)] sm:px-10 sm:pb-14 sm:pt-14 lg:px-14",
        heroGradient(room.theme),
      )}
      style={room.bannerImageUrl ? {
        backgroundImage: `linear-gradient(110deg, rgba(87,64,211,.84), rgba(157,139,245,.74)), url(${room.bannerImageUrl})`,
        backgroundSize: "cover",
        backgroundPosition: "center",
      } : undefined}
    >
      <div className="pointer-events-none absolute -left-24 -top-32 h-72 w-[72%] rotate-[8deg] rounded-[52%] bg-white/10" />
      ...
      <div className="relative z-10 mx-auto flex max-w-5xl flex-col items-center text-center">
        <div className="flex items-center justify-center gap-7 sm:gap-12">
          {room.companyLogoUrl ? (
            <div className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-full border-2 border-white bg-white shadow-[0_14px_40px_rgba(51,34,132,0.18)] sm:h-24 sm:w-24">
              <img src={room.companyLogoUrl} alt={`Logo ${room.companyName}`} className="h-full w-full object-contain p-2" />
            </div>
          ) : (
            <div className="grid h-20 w-20 shrink-0 place-items-center rounded-full border-2 border-white bg-white text-lg font-black tracking-[-0.04em] text-[#4d39b8] shadow-[0_14px_40px_rgba(51,34,132,0.18)] sm:h-24 sm:w-24">
              {initials(room.companyName)}
            </div>
          )}

          <span className={cn("text-4xl font-black tracking-[-0.08em] sm:text-5xl", light ? "text-[#172a32]" : "text-white")}>×</span>
          <GandoMark className="h-20 w-20 sm:h-24 sm:w-24" />
        </div>

        <div className="mt-8 max-w-3xl">
          <div className={cn("text-[10px] font-black uppercase tracking-[0.2em]", light ? "text-[#4d39b8]" : "text-white/75")}>Gando Deal Room</div>
          <h1 className={cn("mt-3 text-2xl font-black tracking-[-0.04em] sm:text-4xl", light ? "text-[#172a32]" : "text-white")}>{room.displayTitle}</h1>
          <p className={cn("mx-auto mt-3 max-w-2xl text-sm leading-6 sm:text-base", light ? "text-[#4c5d63]" : "text-white/80")}>{room.displaySubtitle}</p>
        </div>
      </div>
    </section>
  );
  ...
  return (
    <section data-room-section={id} className="scroll-mt-24 rounded-[22px] border border-[#e4e0f3] bg-white p-5 shadow-[0_12px_36px_rgba(40,31,85,0.05)] sm:p-7">
      <div className="flex items-center gap-3">
        <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#f0edff] text-[#6f58e8]"><Icon className="h-5 w-5" /></span>
        <h2 className="text-lg font-bold tracking-[-0.02em] text-[#172a32]">{title}</h2>
      </div>
  ...
  return (
    <ul className="space-y-2.5">
      {items.map((item, index) => (
        <li key={index} className="flex gap-3 text-sm leading-6 text-[#4c5d63]"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-[#6f58e8]" /><span>{item}</span></li>
      ))}
    </ul>
  );
}
...
export function PublicSDRoomV2({ token }: { token: string }) {
      ...
      <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#f7f9fa] p-5 text-[#172a32]">
        ...
        <Card className="relative w-full max-w-md border-white/80 bg-white p-7 shadow-[0_28px_85px_rgba(55,38,136,0.22)] sm:p-9">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2"><GandoMark className="h-9 w-9" /><div className="text-xl font-black tracking-[-0.04em]">GANDO</div></div>
          ...
          <form onSubmit={unlock} className="mt-6 space-y-4">
            <div className="space-y-2"><Label htmlFor="room-email" className="text-[#394c54]">Email professionnel</Label><Input id="room-email" type="email" required autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} className="border-[#dce5e7] bg-white" placeholder="prenom@entreprise.com" /></div>
            {error ? <p className="text-sm text-rose-600">{error}</p> : null}
            <Button type="submit" className="w-full bg-[#6f58e8] text-white hover:bg-[#5d48d6]" disabled={loading}>{loading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ArrowRight className="mr-2 h-4 w-4" />} Ouvrir la room</Button>
          </form>
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fa] text-[#172a32]">
      ...
      <header className="sticky top-0 z-30 border-b border-[#e8e7ef] bg-white/92 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center gap-4 px-5 py-3 sm:px-7">
          <div className="flex items-center gap-2"><GandoMark className="h-9 w-9" /><div className="text-lg font-black tracking-[-0.04em]">GANDO</div></div>
          <div className="h-6 w-px bg-[#e5e8ea]" />
          <div className="min-w-0 flex-1"><div className="truncate text-sm font-bold">{data.room.companyName} × Gando</div><div className="text-[11px] text-[#829096]">Deal Room partagée · {data.visitorEmail}</div></div>
          <Badge variant="outline" className="hidden border-[#ddd5ff] text-[#5d4bc6] sm:inline-flex">Mis à jour {new Intl.DateTimeFormat("fr-FR", { dateStyle: "medium" }).format(new Date(data.room.updatedAt))}</Badge>
        </div>
      </header>

      <div className="mx-auto max-w-7xl px-5 pb-8 pt-5 sm:px-7 sm:pb-10 sm:pt-6">
        <nav className="flex gap-2 overflow-x-auto rounded-2xl border border-[#e4e0f3] bg-white p-2 shadow-sm" aria-label="Étapes SD">
          {SD_CODES.map(code => (
            <button key={code} type="button" disabled={!available.has(code)} onClick={() => { setActiveStage(code); viewedSections.current.clear(); track("stage_viewed", code); }} className={cn("min-w-36 flex-1 rounded-xl px-3 py-2.5 text-left transition-colors", activeStage === code ? "bg-[#f0edff] text-[#4d39b8]" : available.has(code) ? "text-[#637278] hover:bg-[#f7f8fa]" : "cursor-not-allowed text-[#c1c8cb]")}>
              <div className="text-[10px] font-bold uppercase tracking-wider">{code}</div><div className="mt-0.5 truncate text-xs font-semibold">{SD_STAGE_META[code].title}</div>
            </button>
          ))}
        </nav>
