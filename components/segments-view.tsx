...
import { ArrowDown, ArrowUp, Building2, Eye, EyeOff, ListFilter, Loader2, Pencil, Plus, RefreshCw, Search, Trash2, Users, X } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function SegmentsView() {
  ...
  const companySegments = useMemo(() => lists.filter(segment => segment.objectTypeId === "0-2"), [lists]);
  const orderedCompanyIds = useMemo(() => {
  ...
  function toggleProspection(segment: Segment) {
    if (segment.objectTypeId !== "0-2") return;
    const visible = preferences[segment.listId]?.visible !== false;
    savePreferences({
      ...preferences,
      [segment.listId]: { ...preferences[segment.listId], visible: !visible },
    });
    toast.success(!visible ? `« ${segment.name} » est visible dans Prospection.` : `« ${segment.name} » est masqué dans Prospection.`);
  }

  function moveSegment(segment: Segment, direction: -1 | 1) {
    const index = orderedCompanyIds.indexOf(segment.listId);
    const targetIndex = index + direction;
    if (index < 0 || targetIndex < 0 || targetIndex >= orderedCompanyIds.length) return;
    const reordered = [...orderedCompanyIds];
  ...
  async function rename(segment: Segment) {
    ...
    try {
      ...
      setLists(current => current.map(item => item.listId === segment.listId ? { ...item, name: data.name || nextName } : item));
      setEditingId("");
      setEditingName("");
  ...
  async function remove() {
    ...
    try {
      ...
      if (preferences[segmentToDelete.listId]) {
        const next = { ...preferences };
        delete next[segmentToDelete.listId];
        savePreferences(next);
      }
  ...
  const companyCount = companySegments.length;
  ...
  return (
    <div className="page-shell min-h-screen minari-scrollbar">
      <div className="page-content space-y-6">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Choisissez les segments Entreprise visibles dans Prospection, changez leur ordre et renommez-les directement dans HubSpot.</p>
          </div>
        ...
        <div className="grid gap-3 sm:grid-cols-4">
          ...
          <Card className="p-4"><div className="flex items-center gap-2 text-xs font-medium text-muted-foreground"><Eye size={14} /> Dans Prospection</div><div className="mt-1 text-2xl font-bold">{visibleCompanyCount}</div></Card>
        </div>

        <Card id="nouveau-segment">
          <CardHeader><CardTitle className="text-base">Nouveau segment</CardTitle><CardDescription>La liste sera créée directement dans HubSpot. Les segments Entreprise sont visibles par défaut dans Prospection.</CardDescription></CardHeader>
          <CardContent>
        ...
        <Card className="overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border px-5 py-4">
            <div><h2 className="font-semibold">Segments disponibles</h2><p className="text-xs text-muted-foreground">Les changements de visibilité et d’ordre s’appliquent immédiatement à Prospection.</p></div>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? <TableRow className="hover:bg-transparent"><TableCell colSpan={6} className="h-48 text-center"><Loader2 className="mx-auto animate-spin text-primary" /></TableCell></TableRow> : filteredLists.map(segment => {
                ...
                const visible = preferences[segment.listId]?.visible !== false;
                const orderIndex = orderedCompanyIds.indexOf(segment.listId);
                const editing = editingId === segment.listId;
                return (
                  <TableRow key={segment.listId}>
                    <TableCell>
                      {editing ? (
                        <div className="flex max-w-sm items-center gap-2"><Input autoFocus value={editingName} onChange={event => setEditingName(event.target.value)} onKeyDown={event => { if (event.key === "Enter") { event.preventDefault(); void rename(segment); } if (event.key === "Escape") { setEditingId(""); setEditingName(""); } }} className="h-8" /><Button size="sm" className="h-8" disabled={renamingId === segment.listId} onClick={() => void rename(segment)}>{renamingId === segment.listId ? <Loader2 size={13} className="animate-spin" /> : "OK"}</Button><Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => { setEditingId(""); setEditingName(""); }}><X size={14} /></Button></div>
                      ) : (
                    ...
                    <TableCell>{isCompany ? <Button variant={visible ? "secondary" : "outline"} size="sm" className="h-8 gap-1.5" onClick={() => toggleProspection(segment)}>{visible ? <Eye size={13} /> : <EyeOff size={13} />}{visible ? "Visible" : "Masqué"}</Button> : <span className="text-xs text-muted-foreground">Vue Contacts</span>}</TableCell>
                    <TableCell>{isCompany ? <div className="flex items-center gap-1"><Badge variant="outline">#{orderIndex + 1}</Badge><Button variant="ghost" size="icon" className="h-7 w-7" disabled={orderIndex <= 0} onClick={() => moveSegment(segment, -1)}><ArrowUp size={13} /></Button><Button variant="ghost" size="icon" className="h-7 w-7" disabled={orderIndex < 0 || orderIndex >= orderedCompanyIds.length - 1} onClick={() => moveSegment(segment, 1)}><ArrowDown size={13} /></Button></div> : <span className="text-xs text-muted-foreground">—</span>}</TableCell>
      ...
      <AlertDialog open={Boolean(segmentToDelete)} onOpenChange={open => !open && !deletingId && setSegmentToDelete(null)}>
        <AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Supprimer ce segment ?</AlertDialogTitle><AlertDialogDescription>Le segment « {segmentToDelete?.name} » sera supprimé de HubSpot. Les contacts et entreprises qu’il contient ne seront pas supprimés.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={Boolean(deletingId)}>Annuler</AlertDialogCancel><AlertDialogAction disabled={Boolean(deletingId)} className={buttonVariants({ variant: "destructive" })} onClick={event => { event.preventDefault(); void remove(); }}>{deletingId ? <Loader2 size={15} className="animate-spin" /> : <Trash2 size={15} />}{deletingId ? "Suppression…" : "Supprimer de HubSpot"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent>
      </AlertDialog>
    </div>
  );
}
