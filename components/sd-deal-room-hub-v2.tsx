      ...
      <div className="mx-auto max-w-[1600px] px-5 py-5 lg:px-7 lg:py-6">
        <div className="mb-5 flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">Une dealroom peut être autonome. L’association HubSpot sert uniquement à récupérer automatiquement le contexte commercial.</p>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-xl border border-border bg-card">
          {loading ? (
            ...
            <div className="grid min-h-72 place-items-center px-6 text-center"><div><Building2 className="mx-auto h-9 w-9 text-muted-foreground" /><div className="mt-3 font-bold">Aucune dealroom ici</div><p className="mt-1 text-sm text-muted-foreground">Tu peux en créer une sans aucun deal HubSpot.</p><Button className="mt-4" onClick={openCreate}><Plus className="mr-2 h-4 w-4" /> New dealroom</Button></div></div>
          ) : (
                      ...
                      <tr key={room.id} className="group border-b border-border last:border-b-0 transition-colors hover:bg-muted/25">
                        ...
                        <td className="px-3 py-4">
                          <Link href={`/deal-room/${room.hubspot_deal_id}/sd`} className="flex min-w-[240px] items-center gap-3">
                            <Logo room={room} />
                            <div className="min-w-0"><div className="truncate text-sm font-bold text-foreground group-hover:text-primary">{room.title}</div><div className="mt-0.5 truncate text-xs text-muted-foreground">{room.company_name}</div></div>
                          </Link>
                        </td>
                        <td className="px-3 py-4">
                          {room.crmConnected ? <Badge variant="outline" className="rounded-md border-emerald-400/20 bg-emerald-400/[0.07] text-emerald-300"><Link2 className="mr-1 h-3 w-3" />HubSpot</Badge> : <Badge variant="outline" className="rounded-md text-muted-foreground"><Unlink className="mr-1 h-3 w-3" />Autonome</Badge>}
                        </td>
                        ...
                        <td className="px-3 py-4"><div className="flex items-center justify-end gap-1">{room.crm_link ? <Button variant="ghost" size="icon" className="h-8 w-8" asChild><a href={room.crm_link} target="_blank" rel="noreferrer"><ExternalLink className="h-4 w-4" /></a></Button> : null}<Button variant="ghost" size="icon" className="h-8 w-8" asChild><Link href={`/deal-room/${room.hubspot_deal_id}/sd`}><ArrowRight className="h-4 w-4" /></Link></Button></div></td>
                      </tr>
                    );
                  })}
                </tbody>
              </table>
              <div className="flex items-center justify-between border-t border-border px-4 py-3 text-xs text-muted-foreground"><span>Results : {visibleRooms.length} of {rooms.length}</span><span>CRM facultatif · {availableDeals.length} deal{availableDeals.length > 1 ? "s" : ""} HubSpot disponible{availableDeals.length > 1 ? "s" : ""}</span></div>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-[2px]" onMouseDown={event => { if (event.target === event.currentTarget && !creating) setModalOpen(false); }}>
          <div className="w-full max-w-[560px] overflow-hidden rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex items-center justify-between px-6 pb-4 pt-6">
              <div><h2 className="text-xl font-black tracking-[-0.025em]">Create new dealroom</h2><p className="mt-1 text-xs text-muted-foreground">Seuls le nom et l’organisation sont obligatoires.</p></div>
            ...
            <div className="space-y-4 px-6 pb-6">
              <div><label className="mb-1.5 block text-xs font-semibold text-muted-foreground">Dealroom name*</label><Input value={dealroomName} onChange={event => setDealroomName(event.target.value)} placeholder="ex: Gando × Acme Corp." /></div>
              <div><label className="mb-1.5 block text-xs font-semibold text-muted-foreground">Organization name*</label><Input value={organizationName} onChange={event => setOrganizationName(event.target.value)} placeholder="ex: Acme Corp." /></div>
              ...
              <div className="rounded-xl border border-border bg-muted/20 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-[11px] leading-5 text-muted-foreground">Utile pour récupérer les contacts, appels et prochaines actions. Tu peux créer la room sans CRM.</p>
                    <div className="relative mt-3">
                      <select value={selectedDealId} onChange={event => selectDeal(event.target.value)} className="h-10 w-full appearance-none rounded-md border border-input bg-background px-3 pr-9 text-sm outline-none focus:ring-2 focus:ring-ring">
                        <option value="">Aucun — Dealroom autonome</option>
              ...
              <div><label className="mb-1.5 block text-xs font-semibold text-muted-foreground">CRM link <span className="font-normal">(optionnel)</span></label><Input value={crmLink} onChange={event => setCrmLink(event.target.value)} placeholder="https://app.hubspot.com/…" /></div>
              <div>
                <label className="mb-1.5 block text-xs font-semibold text-muted-foreground">Prospect logo <span className="font-normal">(optionnel)</span></label>
                <div className="flex items-center gap-3"><div className="grid h-12 w-12 shrink-0 place-items-center overflow-hidden rounded-xl border border-dashed border-border bg-muted/30 text-muted-foreground">{logoUrl ? <img src={logoUrl} alt="" className="h-full w-full object-contain p-1.5" /> : organizationName ? <span className="text-xs font-black">{initials(organizationName)}</span> : <ImageIcon className="h-5 w-5" />}</div><Input value={logoUrl} onChange={event => setLogoUrl(event.target.value)} placeholder="URL du logo" /></div>
              </div>
              ...
              <div className="flex items-center justify-between gap-3 pt-2">
                <div className="flex items-center gap-2 text-[11px] text-muted-foreground"><CheckCircle2 className="h-3.5 w-3.5 text-emerald-300" /> SD01 → SD05 sera créé automatiquement.</div>
                <Button onClick={() => void createDealroom()} disabled={creating || !dealroomName.trim() || !organizationName.trim()} className="min-w-24">{creating ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Create</Button>
              </div>
            </div>
          </div>
        </div>
