    ...
    <div className="min-h-screen bg-muted/20 px-4 py-6 lg:px-7 lg:py-8">
      <div className="mx-auto max-w-[1320px] space-y-4">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2"><Badge variant="outline">V{sd01?.version || 1}</Badge>{confirmedMetrics.length ? <Badge variant="outline">{confirmedMetrics.length} métrique{confirmedMetrics.length > 1 ? "s" : ""}</Badge> : null}<Button variant="outline" onClick={() => void save(false)} disabled={Boolean(working)}><Save className="mr-2 h-4 w-4" />Enregistrer</Button><Button onClick={() => void save(true)} disabled={Boolean(working)}>{working === "publish" ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Publier</Button></div>
        </div>
        ...
        <div className="grid gap-5 xl:grid-cols-[minmax(0,900px)_340px] xl:items-start">
          <Card className="overflow-hidden border-border/80 bg-card p-0 shadow-sm">
            ...
            <DocBlock title="Modèle commercial" hint="Un seul bloc d’écriture libre. Écris les conditions comme dans une note ; elles seront ensuite affichées proprement en blocs côté client.">
              <div className="rounded-2xl border border-border bg-muted/20 p-5">
                <div className="mb-3 text-[10px] font-black uppercase tracking-[0.14em] text-primary">Bloc d’écriture</div>
                <FreeTextarea value={(content.businessModel || []).join("\n\n")} onChange={value => update("businessModel", value.trim() ? [value] : [])} rows={10} placeholder={"Décris librement le modèle commercial proposé…\n\nEx. Olympic est partenaire et non client : aucun abonnement.\nLes frais de sécurisation sont payés par le client final.\nConditions proposées pour le pilote : 3 % HT par caution activée."} />
              </div>
            </DocBlock>
    ...
    {versionToDelete ? <div className="fixed inset-0 z-[120] grid place-items-center bg-black/45 p-4 backdrop-blur-[2px]" onMouseDown={() => working !== `delete-${versionToDelete.id}` && setVersionToDelete(null)}>
      <div role="dialog" aria-modal="true" aria-labelledby="delete-version-title" className="w-full max-w-md rounded-2xl border border-border bg-card p-6 shadow-2xl" onMouseDown={event => event.stopPropagation()}>
        <div className="flex items-start gap-4">
          <div className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-destructive/10 text-destructive"><Trash2 className="h-5 w-5" /></div>
          <div>
            <div className="text-[10px] font-black uppercase tracking-[0.14em] text-destructive">Suppression définitive</div>
            <h2 id="delete-version-title" className="mt-1 text-xl font-black tracking-[-0.025em]">Supprimer la version {versionToDelete.version} ?</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">Cette version sera retirée de l’historique du SD01. Cette action est définitive et ne peut pas être annulée.</p>
          </div>
        </div>
        <div className="mt-6 flex justify-end gap-2">
          <Button variant="outline" onClick={() => setVersionToDelete(null)} disabled={working === `delete-${versionToDelete.id}`}>Annuler</Button>
          <Button className="bg-destructive text-destructive-foreground hover:bg-destructive/90" onClick={() => void deleteVersion(versionToDelete)} disabled={working === `delete-${versionToDelete.id}`}>
            {working === `delete-${versionToDelete.id}` ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Trash2 className="mr-2 h-4 w-4" />}
