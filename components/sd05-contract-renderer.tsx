...
function ContractBlock({ block, articleColor }: { block: RenderBlock; articleColor: string }) {
  if (block.kind === "major") return <h2 className="pt-0.5 text-[17px] font-black uppercase leading-[1.25] tracking-[0.02em]" style={{ color: articleColor }}>{block.text}</h2>;
  if (block.kind === "article" || block.kind === "h2") return <h2 className="pt-1 text-[16px] font-black uppercase leading-[1.25] tracking-[0.01em]" style={{ color: articleColor }}>{block.text}</h2>;
  if (block.kind === "h3") return <h3 className="pt-0.5 text-[14px] font-black leading-[1.3] text-[#333333]">{block.text}</h3>;
  if (block.kind === "h4") return <h4 className="pt-0.5 text-[12px] font-bold uppercase leading-[1.3] tracking-[0.035em] text-[#333333]">{block.text}</h4>;
  if (block.kind === "subsection") return <h3 className="pt-0.5 text-[13px] font-black leading-[1.35] text-[#333333]">{block.text}</h3>;
  if (block.kind === "bullet") return <p className="whitespace-pre-line pl-5 text-[12px] leading-[1.48] text-[#333333]">{block.text}</p>;
  if (block.kind === "table") {
    ...
    return <div className="my-2 overflow-hidden rounded-lg border border-slate-300"><table className="w-full border-collapse text-left text-[11px] leading-[1.4] text-[#333333]"><thead className="bg-slate-100"><tr>{head.map((cell, index) => <th key={index} className="border-r border-slate-300 px-3 py-1.5 font-black last:border-r-0">{cell}</th>)}</tr></thead><tbody>{body.map((row, rowIndex) => <tr key={rowIndex} className="border-t border-slate-200">{row.map((cell, cellIndex) => <td key={cellIndex} className="border-r border-slate-200 px-3 py-1.5 align-top last:border-r-0">{cell}</td>)}</tr>)}</tbody></table></div>;
  }
  return <p className="whitespace-pre-line text-[12px] leading-[1.5] text-[#333333]">{block.text}</p>;
}
...
export function SD05ContractRenderer({ content, companyName, contractHash, signatures = [], initialsByPage, compact = false }: { content: SD05Content; companyName?: string; contractHash?: string | null; signatures?: SD05SignatureSummary[]; initialsByPage?: Record<string, string>; compact?: boolean }) {
  ...
  return <div className="space-y-5 print:space-y-0">
    ...
    <PageShell content={content} pageNumber={totalPages} totalPages={totalPages} signatures={signatures} initialsByPage={initialsByPage} compact={compact}>
      <section className="pt-2"><div className="text-[11px] font-black uppercase tracking-[0.14em] text-[#735DF3]">Signature des Parties</div><h2 className="mt-2 text-[25px] font-black tracking-[-0.03em]">Fait et signé électroniquement</h2><p className="mt-2.5 max-w-2xl text-[12px] leading-[1.5] text-slate-600">Chaque Partie reconnaît avoir reçu un exemplaire électronique du contrat. La version signée est figée et reliée au dossier de preuve par son empreinte cryptographique.</p></section>
