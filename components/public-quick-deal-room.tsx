...
import { Check, CheckCircle2, ExternalLink, FileSignature, FileText, Loader2 } from "lucide-react";
    ...
    <div className="mx-auto max-w-[980px] px-5 py-9 sm:px-7 sm:py-12">
      <section className="rounded-[20px] border border-[#e0e4e6] bg-white p-6 sm:p-8">
        ...
        {revenueExample.enabled && revenueExample.averageDeposit > 0 && revenueExample.monthlyActivations > 0 && revenueExample.partnerMarginRate > 0 ? <div className={proposalContent.pricing.length ? "mt-7 border-t border-[#eceeef] pt-6" : ""}>
          ...
          <div className="mt-2 max-w-2xl text-sm leading-6 text-[#687277]">Une illustration concrète de ce que la marge partenaire peut représenter. Cette projection dépend du volume réel de cautions activées. Tous les revenus indiqués sont hors taxes (HT).</div>
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            <div className="rounded-[16px] border border-[#e0e4e6] bg-[#f8f9f9] p-4"><div className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#687277]">Par caution · HT</div><div className="mt-2 text-[27px] font-semibold tracking-[-0.035em] text-[#211f32]">{euro(revenuePerDeposit)} HT</div><div className="mt-1 text-xs text-[#687277]">{euro(revenueExample.averageDeposit)} × {revenueExample.partnerMarginRate.toLocaleString("fr-FR")} %</div></div>
            <div className="rounded-[16px] border border-[#dedcf0] bg-[#f8f7ff] p-4"><div className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#7166c7]">Revenu estimé / mois · HT</div><div className="mt-2 text-[27px] font-semibold tracking-[-0.035em] text-[#6257b8]">{euro(monthlyRevenue)} HT</div><div className="mt-1 text-xs text-[#687277]">{revenueExample.monthlyActivations.toLocaleString("fr-FR")} cautions activées / mois</div></div>
            <div className="rounded-[16px] border border-[#e0e4e6] bg-[#f8f9f9] p-4"><div className="text-[10px] font-bold uppercase tracking-[0.11em] text-[#687277]">Projection annuelle · HT</div><div className="mt-2 text-[27px] font-semibold tracking-[-0.035em] text-[#211f32]">{euro(annualRevenue)} HT</div><div className="mt-1 text-xs text-[#687277]">à volume mensuel constant</div></div>
          </div>
          <div className="mt-4 rounded-xl bg-[#f8f9f9] px-4 py-3 text-sm leading-6 text-[#59646a]">
            Exemple : avec une caution moyenne de <strong className="text-[#1c2529]">{euro(revenueExample.averageDeposit)}</strong> et une marge partenaire de <strong className="text-[#1c2529]">{revenueExample.partnerMarginRate.toLocaleString("fr-FR")} %</strong>, chaque caution activée génère <strong className="text-[#1c2529]">{euro(revenuePerDeposit)} HT</strong>. À <strong className="text-[#1c2529]">{revenueExample.monthlyActivations.toLocaleString("fr-FR")} cautions/mois</strong>, cela représente environ <strong className="text-[#6257b8]">{euro(monthlyRevenue)} HT/mois</strong>.
          </div>
        </div> : null}
      ...
      {agreed ? <section className="mt-6 rounded-[20px] border border-[#e0e4e6] bg-white p-6 sm:p-8"><div className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#7166c7]">Étape suivante</div><h2 className="mt-1 text-2xl font-semibold">Contrat & signature</h2>{hasContract ? <div className="mt-5 rounded-xl border border-[#e1e5e7] bg-[#f8f9f9] p-4"><div className="flex items-start gap-3"><FileText className="mt-0.5 h-6 w-6 shrink-0 text-[#7166c7]" /><div className="min-w-0 flex-1"><div className="truncate font-semibold">{contractContent.contractTitle || "Contrat"}</div><div className="mt-1 text-xs text-[#687277]">{contractSigned ? "Contrat signé" : signatureReady ? "Prêt à signer" : "Contrat disponible"}</div></div>{signatureReady && !contractSigned ? <span className="rounded-full bg-[#f2f0ff] px-2.5 py-1 text-[11px] font-semibold text-[#6257b8]">Signature électronique</span> : null}</div><div className="mt-4 flex flex-wrap gap-2"><a href={publicContractHref} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-[#d2d7da] bg-white px-4 text-sm font-semibold text-[#202a2f]">Voir le contrat <ExternalLink className="h-4 w-4" /></a>{signatureReady && !contractSigned ? <a href={contractContent.signatureUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#202a2f] px-4 text-sm font-semibold text-white"><FileSignature className="h-4 w-4" />Signer le contrat <ExternalLink className="h-4 w-4" /></a> : null}</div>{signatureReady && !contractSigned ? <p className="mt-3 text-xs leading-5 text-[#687277]">La signature s’ouvre dans Signature électronique. Une fois terminée, revenez dans cette Dealroom.</p> : null}</div> : <p className="mt-3 text-sm leading-6 text-[#687277]">Le contrat est en préparation. Vous le retrouverez ici dès qu’il sera ajouté au deal.</p>}</section> : null}
    </div>
  </main>;
}
