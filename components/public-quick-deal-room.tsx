...
export function PublicQuickDealRoom({ token }: { token: string }) {
  ...
  return <main className="min-h-screen bg-[#f5f6f7] text-[#1c2529]">
    ...
    <div className="mx-auto max-w-[980px] px-5 py-9 sm:px-7 sm:py-12">
      ...
      {agreed ? <section className="mt-6 rounded-[20px] border border-[#e0e4e6] bg-white p-6 sm:p-8"><div className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#7166c7]">Étape suivante</div><h2 className="mt-1 text-2xl font-semibold">Contrat & signature</h2>{contractContent.contractUrl ? <div className="mt-5 rounded-xl border border-[#e1e5e7] bg-[#f8f9f9] p-4"><div className="flex items-start gap-3"><FileText className="mt-0.5 h-6 w-6 shrink-0 text-[#7166c7]" /><div className="min-w-0 flex-1"><div className="truncate font-semibold">{contractContent.contractTitle || "Contrat"}</div><div className="mt-1 text-xs text-[#687277]">{contractSigned ? "Contrat signé" : odooReady ? "Prêt à signer" : "Contrat disponible"}</div></div>{odooReady && !contractSigned ? <span className="rounded-full bg-[#f2f0ff] px-2.5 py-1 text-[11px] font-semibold text-[#6257b8]">Odoo Signature</span> : null}</div><div className="mt-4 flex flex-wrap gap-2"><a href={contractContent.contractUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-[#d2d7da] bg-white px-4 text-sm font-semibold text-[#202a2f]">Voir le contrat <ExternalLink className="h-4 w-4" /></a>{odooReady && !contractSigned ? <a href={contractContent.signatureUrl} target="_blank" rel="noreferrer" className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-[#202a2f] px-4 text-sm font-semibold text-white"><FileSignature className="h-4 w-4" />Signer le contrat <ExternalLink className="h-4 w-4" /></a> : null}</div>{odooReady && !contractSigned ? <p className="mt-3 text-xs leading-5 text-[#687277]">La signature s’ouvre dans Odoo Signature. Une fois terminée, revenez dans cette Dealroom.</p> : null}</div> : <p className="mt-3 text-sm leading-6 text-[#687277]">Le contrat est en préparation. Vous le retrouverez ici dès qu’il sera ajouté au deal.</p>}</section> : null}
    </div>
  </main>;
}
