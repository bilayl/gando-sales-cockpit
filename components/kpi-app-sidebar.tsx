...
export function KpiAppSidebar({
  ...
  return (
    <aside className="flex h-screen w-[72px] flex-col border-r border-border bg-card px-2.5 py-4 lg:w-[224px] lg:px-3">
      <button
        type="button"
        onClick={() => onViewChange("ceo")}
      ...
      >
        ...
        <span className="hidden min-w-0 lg:block">
          ...
          <span className="block text-[9px] font-bold uppercase tracking-[0.17em] text-muted-foreground">CEO Cockpit</span>
        </span>
      </button>
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto minari-scrollbar" aria-label="Navigation KPI">
        <div className="space-y-4">
          {groups.map(group => (
            <div key={group.label}>
              <div className="mb-1 hidden px-2.5 text-[9px] font-bold uppercase tracking-[0.16em] text-muted-foreground/60 lg:block">
                {group.label}
              </div>
