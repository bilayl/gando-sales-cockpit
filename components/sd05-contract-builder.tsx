...
export function SD05ContractBuilder({ dealId }: { dealId: string }) {
  ...
  function applyTemplate(templateId: SD05TemplateId) {
    ...
    setValue(current => ({ ...template, contractReference: current.contractReference || template.contractReference, signatureDeadline: current.signatureDeadline, goLiveDate: current.goLiveDate, effectiveDate: current.effectiveDate, signatureProvider: current.signatureProvider, contractUrl: current.contractUrl, signatureUrl: current.signatureUrl }));
