...
type RoomResponse = {
  ...
  room: { id: string; title: string; company_name?: string } | null;
};
...
function statusClass(status: string) {
  ...
  if (status === "failed" || status === "expired" || status === "revoked") return "border-red-500/20 bg-red-500/[0.05] text-red-700";
...
export function SD05ContractBuilder({ dealId }: { dealId: string }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const [roomResponse, signaturesResponse] = await Promise.all([
      ...
      setValue({ ...createEmptySD05(), ...raw });

      const signaturesPayload = await signaturesResponse.json().catch(() => ({}));
      if (signaturesResponse.ok) setSignatures((signaturesPayload as SignaturesResponse).requests || []);
      else setSignatures([]);
    } catch (error) {
  ...
  async function persist(publish: boolean, successToast = true) {
    const content: SD05Content = {
      ...
      signatories: value.signatories
        .map(item => ({ ...item, name: item.name.trim(), email: item.email.trim().toLowerCase(), role: item.role.trim(), organization: item.organization.trim() }))
    ...
    return payload.document as SDDocumentRecord;
  }
  ...
  async function sendForSignature(signerEmail?: string) {
    if (working || locked) return;
    if (!requiredReady) {
      toast.error("SD01 et SD02 doivent être validés avant l'envoi en signature.");
      return;
    }
    const validSigners = value.signatories.filter(item => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(item.email.trim()));
    if (!validSigners.length) {
      toast.error("Ajoutez au moins un signataire avec une adresse email valide.");
      return;
    }
    setWorking(true);
    try {
      await persist(true, false);
      const response = await fetch(`/api/deals/${encodeURIComponent(dealId)}/sd-room/sd05-signatures`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(signerEmail ? { signerEmail } : {}),
      });
      const payload = await response.json().catch(() => ({}));
      ...
      if (payload.sent?.length) toast.success(`${payload.sent.length} invitation(s) de signature envoyée(s).`);
      await load();
    } catch (error) {
  ...
  function applyTemplate(templateId: SD05TemplateId) {
    ...
    setValue(current => ({
      ...
      signatureDeadline: current.signatureDeadline,
  ...
  const updateSigner = (index: number, key: "name" | "role" | "organization" | "email", next: string) => setValue(current => ({ ...current, signatories: current.signatories.map((item, currentIndex) => currentIndex === index ? { ...item, [key]: next } : item) }));
  ...
  const updateTerm = (index: number, key: "topic" | "owner" | "notes", next: string) => setValue(current => ({ ...current, legalItems: current.legalItems.map((item, currentIndex) => currentIndex === index ? { ...item, [key]: next } : item) }));
    ...
    <div className="page-shell min-h-screen p-5 lg:p-7">
      <div className="mx-auto max-w-[1180px] space-y-5">
        <Card className="overflow-hidden p-0">
          ...
          {!requiredReady ? <div className="border-b border-amber-500/20 bg-amber-500/[0.06] px-5 py-3 text-xs text-amber-700">SD01 et SD02 doivent être validés avant l'envoi du contrat en signature.</div> : null}
          {locked ? <div className="border-b border-emerald-500/20 bg-emerald-500/[0.05] px-5 py-3 text-xs text-emerald-700">Cette version est signée : le contenu est verrouillé pour préserver l'intégrité de la preuve.</div> : null}
        </Card>
          ...
          <div className="space-y-5">
            ...
            <Card className="space-y-5 p-5">
              <div className="grid gap-4 md:grid-cols-2">
                ...
                <Field label="Mise en production"><Input disabled={locked} type="date" value={value.goLiveDate} onChange={event => set("goLiveDate", event.target.value)} /></Field>
                <Field label="Durée initiale"><Input disabled={locked} value={value.term} onChange={event => set("term", event.target.value)} placeholder="12 mois à compter de la mise en production" /></Field>
              </div>
              <Field label="Renouvellement"><Input disabled={locked} value={value.renewal} onChange={event => set("renewal", event.target.value)} /></Field>
              <Field label="Préavis / résiliation"><Input disabled={locked} value={value.terminationNotice} onChange={event => set("terminationNotice", event.target.value)} /></Field>
            </Card>
            ...
            <Card className="space-y-4 p-5">
              <div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Signataires</h2><p className="mt-1 text-xs text-muted-foreground">Une invitation personnelle est créée pour chaque adresse email renseignée.</p></div>{!locked ? <Button type="button" variant="outline" size="sm" onClick={addSigner}>Ajouter un signataire</Button> : null}</div>
              {value.signatories.length ? <div className="space-y-3">{value.signatories.map((signer, index) => {
                const evidence = signatures.find(item => item.signerEmail.toLowerCase() === signer.email.toLowerCase() && !["revoked", "failed"].includes(item.status));
                return (
                  <div key={index} className="rounded-xl border border-border bg-muted/20 p-4">
                    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-[1fr_1.2fr_1fr_1fr_auto]">
                      <Input disabled={locked} value={signer.name} onChange={event => updateSigner(index, "name", event.target.value)} placeholder="Nom" />
                      <Input disabled={locked} type="email" value={signer.email} onChange={event => updateSigner(index, "email", event.target.value)} placeholder="Email" />
                      <Input disabled={locked} value={signer.role} onChange={event => updateSigner(index, "role", event.target.value)} placeholder="Fonction" />
                      <Input disabled={locked} value={signer.organization} onChange={event => updateSigner(index, "organization", event.target.value)} placeholder="Organisation" />
                      {!locked ? <Button type="button" variant="ghost" size="icon" onClick={() => removeSigner(index)} aria-label="Supprimer"><Trash2 className="h-4 w-4" /></Button> : null}
                    </div>
                    <div className="mt-3 flex flex-wrap items-center gap-2">
                      {evidence ? <Badge variant="outline" className={statusClass(evidence.status)}>{statusLabel(evidence.status)}</Badge> : <Badge variant="outline">À préparer</Badge>}
              ...
              })}</div> : <div className="rounded-xl border border-dashed border-border p-6 text-center text-sm text-muted-foreground">Aucun signataire pour le moment.</div>}
            </Card>
          </div>
