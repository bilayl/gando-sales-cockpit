...
function ContractTextEditor({ value, onChange, disabled, onAddAnnex }: { value: string; onChange: (value: string) => void; disabled: boolean; onAddAnnex: () => void }) {
  ...
  return <div className="overflow-hidden rounded-xl border border-input bg-background focus-within:border-primary focus-within:ring-2 focus-within:ring-primary/15">
    {!disabled ? <div className="flex flex-wrap items-center gap-1.5 border-b border-border bg-muted/25 p-2">
      ...
      <Button type="button" variant="outline" size="sm" onClick={insertTable}>+ Tableau</Button><Button type="button" variant="outline" size="sm" className="ml-auto" onClick={onAddAnnex}>+ Annexe</Button>
    </div> : null}
    <textarea
      ref={ref}
      value={value}
...
export function SD05ContractBuilder({ dealId }: { dealId: string }) {
  ...
  const usesDocumenso = value.signatureProvider === "documenso";
  ...
  async function persist(publish: boolean, successToast = true) {
    const content: SD05Content = {
      ...value,
      contractTitle: value.contractTitle.trim(), contractReference: value.contractReference.trim(), contractVersion: value.contractVersion.trim(), contractSummary: value.contractSummary.trim(), contractUrl: value.contractUrl.trim(), signatureUrl: value.signatureUrl.trim(),
    ...
    if (publish && content.signatureProvider === "documenso" && !/^https?:\/\//i.test(content.signatureUrl)) throw new Error("Ajoutez un lien Documenso valide avant de publier le contrat.");
  ...
  async function sendForSignature(signerEmail?: string) {
    if (working || locked) return;
    if (value.signatureProvider === "documenso") return toast.error("Ce contrat utilise Documenso. Publiez-le puis utilisez le lien Documenso depuis la Dealroom.");
  ...
  function applyTemplate(templateId: SD05TemplateId) {
    if (locked) return;
    const template = templateId === "legal_convention"
  ...
  return <div className="page-shell min-h-screen p-5 lg:p-7"><div className="mx-auto max-w-[1180px] space-y-5">
    <Card className="overflow-hidden p-0"><div className="flex flex-col gap-4 border-b border-border bg-primary/[0.04] p-5 lg:flex-row lg:items-center"><div className="flex items-start gap-3"><div className="rounded-xl bg-primary/10 p-2.5 text-primary"><FileSignature className="h-5 w-5" /></div><div><div className="text-xs font-bold uppercase tracking-[0.14em] text-primary">SD05 · Contrat & signature électronique</div><h1 className="mt-1 text-2xl font-bold tracking-[-0.03em]">Contrats juridiques Gando</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Mise en page juridique, paraphes, signature manuscrite ou écrite et preuve horodatée.</p></div></div><div className="flex flex-wrap items-center gap-2 lg:ml-auto">{locked ? <Badge variant="outline" className="border-emerald-500/30 text-emerald-600"><CheckCircle2 className="mr-1 h-3.5 w-3.5" /> Signé · version figée</Badge> : null}{value.contractSummary ? <Button variant="outline" asChild><a href={`/api/deals/${encodeURIComponent(dealId)}/sd-room/sd05-pdf`} target="_blank" rel="noreferrer"><Download className="mr-2 h-4 w-4" /> Aperçu PDF</a></Button> : null}<Button variant="outline" onClick={() => void save(false)} disabled={working || locked}><Save className="mr-2 h-4 w-4" /> Enregistrer</Button>{usesDocumenso ? <Button onClick={() => void save(true)} disabled={working || locked || !/^https?:\/\//i.test(value.signatureUrl.trim())}><Send className="mr-2 h-4 w-4" /> Publier pour signature</Button> : <Button onClick={() => void sendForSignature()} disabled={working || locked || !requiredReady}><Send className="mr-2 h-4 w-4" /> Envoyer pour signature</Button>}</div></div>{!requiredReady && !usesDocumenso ? <div className="border-b border-amber-500/20 bg-amber-500/[0.06] px-5 py-3 text-xs text-amber-700">SD01 et SD02 doivent être validés avant l'envoi du contrat en signature Gando.</div> : null}</Card>

    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_350px]"><div className="space-y-5">
      <Card className="space-y-4 p-5"><div><h2 className="font-semibold">Modèle de contrat</h2><p className="mt-1 text-xs text-muted-foreground">Choisis le document à générer. Le modèle Loueur reprend le SD05 de référence sur 12 pages.</p></div><div className="grid gap-3 md:grid-cols-3"><button type="button" disabled={locked} onClick={() => applyTemplate("rental_exact")} className={value.contractTemplate === "rental_exact" ? "rounded-xl border-2 border-primary bg-primary/5 p-4 text-left" : "rounded-xl border border-border p-4 text-left hover:bg-muted/30"}><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-primary" /> Contrat loueur SD05</div><p className="mt-1 text-xs text-muted-foreground">Même template que le PDF de référence : 12 pages, annexes et tarification.</p></button><button type="button" disabled={locked} onClick={() => applyTemplate("gando_standard")} className={value.contractTemplate === "gando_standard" ? "rounded-xl border-2 border-primary bg-primary/5 p-4 text-left" : "rounded-xl border border-border p-4 text-left hover:bg-muted/30"}><div className="flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-primary" /> Services Gando</div><p className="mt-1 text-xs text-muted-foreground">Structure Gando générique.</p></button><button type="button" disabled={locked} onClick={() => applyTemplate("legal_convention")} className={value.contractTemplate === "legal_convention" ? "rounded-xl border-2 border-primary bg-primary/5 p-4 text-left" : "rounded-xl border border-border p-4 text-left hover:bg-muted/30"}><div className="flex items-center gap-2 text-sm font-semibold"><FileSignature className="h-4 w-4 text-primary" /> Convention juridique</div><p className="mt-1 text-xs text-muted-foreground">Convention partenaire institutionnelle.</p></button></div></Card>

      {value.contractTemplate === "rental_exact" ? <Card className="space-y-4 p-5"><div><h2 className="font-semibold">Informations du Loueur</h2><p className="mt-1 text-xs text-muted-foreground">Injectées dans la première page et dans les clauses/annexes du modèle.</p></div><div className="grid gap-4 md:grid-cols-2">
        <Field label="Raison sociale"><Input disabled={locked} value={value.rentalTemplate.legalName} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, legalName: event.target.value } }))} /></Field>
        <Field label="Forme juridique"><Input disabled={locked} value={value.rentalTemplate.legalForm} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, legalForm: event.target.value } }))} /></Field>
        <Field label="Capital social"><Input disabled={locked} value={value.rentalTemplate.shareCapital} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, shareCapital: event.target.value } }))} placeholder="5 000,00 €" /></Field>
        <Field label="SIREN"><Input disabled={locked} value={value.rentalTemplate.siren} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, siren: event.target.value } }))} /></Field>
        <Field label="N° TVA"><Input disabled={locked} value={value.rentalTemplate.vatNumber} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, vatNumber: event.target.value } }))} /></Field>
        <Field label="Email"><Input disabled={locked} value={value.rentalTemplate.contactEmail} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, contactEmail: event.target.value } }))} /></Field>
        <div className="md:col-span-2"><Field label="Adresse du siège"><Input disabled={locked} value={value.rentalTemplate.registeredOffice} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, registeredOffice: event.target.value } }))} /></Field></div>
        <Field label="Territoire / activité"><Input disabled={locked} value={value.rentalTemplate.activityRegion} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, activityRegion: event.target.value } }))} /></Field>
        <Field label="Tarif Gando (%)"><Input disabled={locked} value={value.rentalTemplate.gandoRate} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, gandoRate: event.target.value } }))} /></Field>
        <Field label="Marge loueur (%)"><Input disabled={locked} value={value.rentalTemplate.partnerRate} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, partnerRate: event.target.value } }))} /></Field>
        <Field label="Tarif total client (%)"><Input disabled={locked} value={value.rentalTemplate.totalRate} onChange={event => setValue(current => ({ ...current, rentalTemplate: { ...current.rentalTemplate, totalRate: event.target.value } }))} /></Field>
      </div></Card> : null}
      ...
      <Card className="space-y-4 p-5">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <button type="button" disabled={locked} onClick={() => set("signatureProvider", "gando")} className={!usesDocumenso ? "rounded-xl border-2 border-primary bg-primary/5 p-4 text-left" : "rounded-xl border border-border p-4 text-left hover:bg-muted/30"}>
