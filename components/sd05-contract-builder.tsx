...
export function SD05ContractBuilder({ dealId }: { dealId: string }) {
  ...
  async function persist(publish: boolean, successToast = true) {
    ...
    if (publish && content.signatureProvider === "odoo" && !/^https?:\/\//i.test(content.signatureUrl)) throw new Error("Ajoutez un lien Odoo Signature valide avant de publier le contrat.");
