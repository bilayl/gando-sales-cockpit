...
function looksLikeEmail(value: string) { return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value); }
...
async function listRequests(documentId: string) {
  const { data, error } = await getSupabaseAdmin().from("sd_contract_signature_requests").select("*").eq("document_id", documentId).order("created_at", { ascending: false }).limit(100);
  if (error) throw error;
  return (data || []).map(item => signatureRequestSummary(item as SignatureRequestRow));
}

function emailHtml(input: { logoUrl: string; signingUrl: string; signerLabel: string; companyName: string; title: string; reference: string; intro: string; expiresAt: string; contractHash: string; pageCount: number; legalTemplate: boolean }) {
  const header = input.legalTemplate ? "#323232" : "#735DF3";
  return `<!doctype html><html><body style="margin:0;padding:0;background:#f4f5f8;font-family:Arial,Helvetica,sans-serif;color:#111827">
<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f4f5f8;padding:28px 12px"><tr><td align="center">
...
<tr><td style="height:46px;background:${header};text-align:center;vertical-align:bottom"><img src="${escapeHtml(input.logoUrl)}" width="52" height="52" alt="Gando" style="display:block;margin:0 auto -26px auto;width:52px;height:52px;border:0" /></td></tr>
<tr><td style="padding:42px 42px 18px;text-align:center"><div style="font-size:12px;font-weight:800;letter-spacing:.14em;color:#735DF3;text-transform:uppercase">Signature électronique Gando</div><h1 style="margin:12px 0 0;font-size:27px;line-height:1.2;letter-spacing:-.03em;color:#111827">Document à signer</h1></td></tr>
<tr><td style="padding:4px 42px 0;font-size:15px;line-height:1.7;color:#4b5563"><p style="margin:0 0 16px">Bonjour <strong style="color:#111827">${escapeHtml(input.signerLabel)}</strong>,</p><p style="margin:0 0 16px">${escapeHtml(input.intro)}</p></td></tr>
<tr><td style="padding:4px 42px 8px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:12px"><tr><td style="padding:18px 20px;font-size:13px;line-height:1.7;color:#475569"><div style="font-size:16px;font-weight:800;color:#111827">${escapeHtml(input.title)}</div>${input.reference ? `<div style="margin-top:5px;color:#735DF3;font-weight:700">${escapeHtml(input.reference)}</div>` : ""}<div style="margin-top:12px"><strong>Organisation :</strong> ${escapeHtml(input.companyName)}</div><div><strong>Pages :</strong> ${input.pageCount}</div><div><strong>Expiration :</strong> ${new Date(input.expiresAt).toLocaleDateString("fr-FR")}</div></td></tr></table></td></tr>
<tr><td align="center" style="padding:24px 42px"><a href="${escapeHtml(input.signingUrl)}" style="display:inline-block;background:#735DF3;color:#ffffff;text-decoration:none;padding:14px 28px;border-radius:10px;font-size:14px;font-weight:800">Consulter et signer le contrat</a><div style="margin-top:12px;font-size:11px;line-height:1.5;color:#94a3b8">Ce lien personnel ouvre directement le contrat dans votre Gando Deal Room.</div></td></tr>
<tr><td style="padding:0 42px 28px"><table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background:#111827;border-radius:12px"><tr><td style="padding:18px 20px;color:#ffffff"><div style="font-size:12px;font-weight:800">Intégrité & preuve</div><div style="margin-top:7px;font-size:11px;line-height:1.6;color:#cbd5e1">Le contrat est figé par empreinte SHA-256. Les consultations, paraphes, choix de signature et horodatages sont conservés dans le journal d'audit.</div><div style="margin-top:10px;word-break:break-all;font-family:monospace;font-size:9px;line-height:1.5;color:#94a3b8">${input.contractHash}</div></td></tr></table></td></tr>
<tr><td style="padding:20px 42px 28px;border-top:1px solid #e5e7eb;text-align:center;font-size:10px;line-height:1.6;color:#94a3b8">GANDO SOLUTIONS · RCS Meaux 943 391 201 · 3 chemin de la porte verte, 77144 Montévrain<br />Besoin d'aide ? Répondez à cet email ou contactez contact@gando.app.</td></tr>
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (content.signatureProvider === "documenso") throw Object.assign(new Error("Ce contrat utilise Documenso. Utilisez le parcours de signature automatisé configuré pour ce contrat."), { status: 409 });
    ...
    signers = [...new Map(signers.map(item => [email(item.email), item])).values()];
    ...
    for (const signer of signers) {
      ...
      const { data: activeRows, error: activeError } = await admin.from("sd_contract_signature_requests").select("id").eq("document_id", document.id).eq("signer_email", signerEmail).in("status", ["pending", "sent", "viewed"]);
      ...
      const { data: created, error: createError } = await admin.from("sd_contract_signature_requests").insert({
        room_id: bundle.room.id, document_id: document.id, signer_name: signer.name || signerEmail, signer_email: signerEmail, signer_role: signer.role || null, signer_organization: signer.organization || bundle.room.company_name,
        token_hash: hashSignatureToken(rawToken), contract_reference: content.contractReference || null, contract_version: content.contractVersion || null, contract_snapshot: snapshot, contract_hash: contractHash,
        status: "pending", consent_text: SD05_SIGNATURE_CONSENT, document_page_count: pageCount, expires_at: expiresAt, created_by_email: userEmail,
      }).select("*").single();
      ...
      const body = [`Bonjour ${signerLabel},`, "", intro, "", content.contractTitle, content.contractReference ? `Référence : ${content.contractReference}` : "", `Nombre de pages : ${pageCount}`, "", `Consulter et signer : ${signingUrl}`, "", `Ce lien est personnel et valable jusqu'au ${new Date(expiresAt).toLocaleDateString("fr-FR")}.`, `Empreinte SHA-256 : ${contractHash}`, "", "Après signature, Gando conserve le document figé, les paraphes, le mode de signature, les horodatages et le journal d'audit comme éléments de preuve."].filter(Boolean).join("\n");
      const htmlBody = emailHtml({ logoUrl, signingUrl, signerLabel, companyName: bundle.room.company_name, title: content.contractTitle, reference: content.contractReference, intro, expiresAt, contractHash, pageCount, legalTemplate: content.contractTemplate === "legal_convention" });

      try {
        const emailResult = await sendSmtp2goEmail({ to: signerEmail, subject, body, htmlBody, replyTo: "contact@gando.app" });
        const sentAt = new Date().toISOString();
        const { error: sentError } = await admin.from("sd_contract_signature_requests").update({ status: "sent", sent_at: sentAt, smtp_provider_message_id: emailResult.emailId, smtp_request_id: emailResult.requestId }).eq("id", createdRow.id);
    ...
    if (sent.length) {
      ...
      const { error: documentError } = await admin.from("sd_documents").update({ content: nextContent, published_content: nextContent, status: "published", published_at: document.published_at || new Date().toISOString() }).eq("id", document.id);
